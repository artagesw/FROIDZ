...
/**
 * An Obstacle represents a solid polygon and deals with collisions.
 * 
 * Obstacles are stationary and indestructable.
 * 
 * @author Jacob Weiss
 * @version 0.3.0
 */
    ...
    {        
        ...
        for (Collidable actor : collisions)
        {
            ...
            if (normal != null)
            {
                ...
                Vector w = this.extend(normal, state);
                ...
                state.getVelocity().add(normalComp).add(w);
                state.act();
            }
        }
    }
